...
Validator.registerAsync('exist', function (value, attribute, req, passes) {
    if (!attribute) throw new Error('Specify Requirements i.e fieldName: exist:table,column,required');
    ...
    const { 0: table, 1: column, 2: required } = attArr;

    console.log(required);

    //define custom error message
    let msg = !required ? ((column == "username") ? `${column} has already been taken ` : `${column} already in use`) : `${column} isn't existed in the database`

    //check if incoming value already exists in the database
    Models[table].findOne({ where: { [column]: value } })
        .then((result) => {
            if (result && !required) {
            ...
            if (!result && required) {
                passes(false, msg); // return false if value isn't exists
                return;
            }
            passes();
        })
